...
indexDorm = j;
    ...
    if (students[indexStud].dorm == NULL) {
    assign_student(&students[indexStud], &dorms[indexDorm], id, dorm_name);
} else {
    for (int i = 0; i < dr; i++) {
        if (strcmp(students[indexStud].dorm->name, dorms[i].name) == 0) {
            move_student(&students[indexStud], &dorms[indexDorm], &dorms[i], id, dorm_name);
        }
    }
}
        }
