...
void print_dorm1(struct dorm_t *_dorm, int quant)
{
    for(int i = 0; i < quant; ++i)
    {
        
        printf("%s|%d|%s", _dorm[i].name, _dorm[i].capacity, _dorm[i].gender == 0 ? "male" : "female");
    }
}

void print_dorm_detail1(struct dorm_t *_dorm, int quant)
{
    for (int i = 0; i < quant; ++i)
    {
        printf("%s|%d|%s|%d", _dorm[i].name, _dorm[i].capacity, _dorm[i].gender == 0 ? "male" : "female", _dorm[i].residents_num);
